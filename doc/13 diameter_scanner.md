...
#### 8.2.2 子模式2：扫描边缘校准（编码器值显示）
- **功能**：记录并显示4个扫描点的上升沿和下降沿编码器值，用于诊断和校准扫描点安装位置
- **实现方法**：`displayScannerEncoderValues()`
- **调用方式**：在main函数中，当处于MODE_DIAGNOSE_SCANNER模式的子模式2时调用此方法
- **控制变量**：`scanEdgeCalibrationMode`（扫描边缘校准模式标志）
- **控制方法**：
  - `enableScanEdgeCalibration()`：启用扫描边缘校准模式
  - `disableScanEdgeCalibration()`：禁用扫描边缘校准模式
- **输出方式**：
  - 显示4个扫描点的上升沿编码器值（横排）
  - 显示4个扫描点的下降沿编码器值（下排）
  - 每个数据占3个字符宽度，右对齐
  - 格式为：
    ```
    Rising: [000] [000] [000] [000]
    Falling:[000] [000] [000] [000]
    ```
  - 通过SSD1306 OLED显示器显示
...
- **调用方式**：在main函数中，当处于MODE_DIAGNOSE_SCANNER模式的子模式3时调用此方法
- **输出方式**：
  - 显示4个扫描点的原始直径值
  - 通过串口输出直径数据
