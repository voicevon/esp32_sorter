...
// 出口数量定义
static const int SORTER_NUM_OUTLETS = 8;
...
private:
    ...
    // 状态标志位（中断安全）
    volatile bool restartScan;          // 重启扫描标志 - 重置扫描仪状态并启动新的扫描过程
    volatile bool calculateDiameter;      // 计算直径数据标志 - 在特定编码器相位触发直径计算和处理
...
public:
    ...
    // 静态回调函数，用于编码器相位变化
    static void encoderPhaseCallback(void* context, int phase);
